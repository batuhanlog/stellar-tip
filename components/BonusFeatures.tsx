...
export function ThemeToggle() {
  ...
  return (
    ...
    >
      {isDark ? (
        <FaSun className="text-violet-400 text-lg" />
      ) : (
        <FaMoon className="text-violet-600 text-lg" />
      )}
    </button>
  );
}
  ...
  return (
    <div>
      <button
        ...
        className="text-violet-400 hover:text-violet-300 flex items-center gap-2 text-sm transition-colors"
      >
      ...
      {showQR && (
        <div className="mt-4 p-5 bg-white rounded-2xl animate-fadeInScale inline-block">
          <div className="text-center">
            ...
            <button
              onClick={handleCopy}
              className="mt-2 text-violet-600 hover:text-violet-700 text-xs flex items-center gap-1 mx-auto transition-colors"
            >
      ...
      >
        <div className="p-6">
          <div className="text-center mb-6">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-violet-500/15 border border-violet-500/30 mb-4">
          ...
          <div className="space-y-3 mb-6">
            ...
            <div className="bg-gradient-to-r from-violet-500/10 to-purple-500/10 rounded-xl p-4 border border-violet-500/20">
              <p className="text-violet-300/80 text-xs uppercase tracking-wider mb-1">Amount</p>
              <p className="text-white text-3xl font-bold">{amount} <span className="text-violet-400 text-xl">XLM</span></p>
            </div>
            {memo && (
              <div className="bg-white/5 rounded-xl p-4 border border-white/10">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={onConfirm}
              className="flex-1 bg-gradient-to-r from-violet-500 to-purple-500 hover:from-violet-600 hover:to-purple-600 text-white font-bold py-3.5 px-6 rounded-xl transition-all shadow-lg shadow-violet-500/20"
            >
              Send Tip 💸
            </button>
          </div>
...
export function TipLinkGenerator({ publicKey }: { publicKey: string }) {
  ...
  return (
    <div className="glass rounded-2xl p-5 border border-violet-500/20 bg-gradient-to-r from-violet-500/5 to-purple-500/5 animate-fadeIn">
